...
def get_estadisticas_descriptivas(request, **kwargs):     
    anio_busqueda= request.GET.get("year", None)
    print(anio_busqueda)
    if(anio_busqueda is None):
        return JsonResponse(data={'mensaje': "parametro year faltante"}, status=400)
    else:            
        try:
            int(anio_busqueda)            
        except ValueError:
            return JsonResponse(data={'mensaje': "parametro year no tiene formato entero"}, status=400)

        if(int(anio_busqueda)>9999):
           return JsonResponse(data={'mensaje': "year no valido <9999"}, status=400)              
    
    datos_fecha=Data.objects.filter( time__year=anio_busqueda).values('measurement_id__name').annotate(    # filtro por  por año
    fecha=TruncDate('time')  # Trunca la fecha y hora a solo la fecha
    ).values('fecha','station','measurement_id__name').annotate(    # Realiza agrupacion 'group by'
    promedio=Avg('value'),Max=Max('value'),Min=Min('value')  # Calcula el promedio de los puntos para cada fecha
    ).order_by('fecha')   # Ordenacion por Fecha
 
       
    return JsonResponse(list(datos_fecha), safe=False,json_dumps_params={'indent': 4})
